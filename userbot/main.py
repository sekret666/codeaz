...
try:
    ...
    PLUGIN_MESAJLAR = {}
    ORJ_PLUGIN_MESAJLAR = {"alive": "`CodeBot Aktivdir`", "afk": f"`{str(choice(AFKSTR))}`", "kickme": "`Yaxsı mən getdim `🥰", "pm": UNAPPROVED_MSG, "dızcı": str(choice(DIZCILIK_STR)), "ban": "{mention}`, banlandı!`", "mute": "{mention}`, susduruldu!`", "approve": "{mention}`, mənə mesaj göndərə bilərsən!`", "disapprove": "{mention}`, artıq mənə mesaj göndərə bilmərsən!`", "block": "{mention}`, bloklandın!`"}
...
LOGS.info(f"Bot versiyası: Codeaz {COD_VERSION}")
