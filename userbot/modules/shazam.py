...
async def shazam(event):
    if not event.is_reply:
        ...
        await event.edit('`🛠 Səs faylı fingerprint formatına çevirilir...`')
        ...
        while True:
            ...
            if results['matches']:
                ...
                await event.edit(f'`İlk {(signature_generator.samples_processed / 16000)} saniyədə heç bir şey tapılmadı... Birazda cəhd edirəm.`')
        
        if not 'track' in sarki:
            return await event.edit('`Təəsüf edirəm 😪 Shazam verdiyiniz səsi anlaya bilmədi 😔. Biraz daha aydın səs göndərə bilərsənmi?`')
        await event.edit('`✅ Musiqini tapdım... Məlumatlar gətirilir...`')
