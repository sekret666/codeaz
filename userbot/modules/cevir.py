...
async def cevir(event):
    ...
    if islem == "foto" or islem == "photo":
        ...
        await event.client.send_file(event.chat_id, "sticker.png", reply_to=rep_msg, caption="@Codeaz `ilə fotoya çevrildi.`")
        ...
        if efekt in EFEKTLER:
            ...
            await event.client.send_file(event.chat_id, "output.mp3", reply_to=rep_msg, caption="@Codeaz `ilƏ efekt edildi.`")
        ...
        try:
            await event.client.send_file(event.chat_id, "out.gif",reply_to=rep_msg, caption=LANG['WITH_COD_GIF'])
        except:
        ...
        try:
            await event.client.send_file(event.chat_id, "out.mp3",reply_to=rep_msg, caption=LANG['WITH_COD_SOUND'])
        except:
            await event.edit(LANG['ERROR'])
            await event.delete()
            os.remove("out.mp3")
