...
async def gdrive_upload_function(dryb):
    ...
    if "|" in input_str:
        ...
        if os.path.exists(input_str):
            ...
            await dryb.edit(
                "Serverdə fayl tapılmadı. Xaiş doğru fayl yerini seçin.")
            return False
    ...
    if required_file_name:
        ...
        try:
            g_drive_link = await upload_file(http, required_file_name,
                                             ...
                                             parent_id)
            await dryb.edit(
                f"Fayl :`{required_file_name}`\nUpload uğurlu! \nEndirmə linki: [Google Drive]({g_drive_link})!"
            )
        except Exception as e:
            await dryb.edit(
...
async def upload_dir_to_gdrive(event):
    ...
    if os.path.isdir(input_str):
        ...
        await event.edit(f"Google Drive linki [burada]({dir_link})")
    else:
        await event.edit(f"{input_str} qovluğu tapılmadı.")


@register(pattern=r"^.glist(?: |$)(.*)", outgoing=True)
...
async def download(set):
    ...
    if input_str:
        parent_id = input_str
        await set.edit(
            "Özəl qovluq ID'si uğurla edildi. Sonrakı uploadlar buraya uploadlanacaq: {parent_id} (`.gsetclear` əmrini vermədiyiniz vaxta qədər.)"
        )
        await set.delete()
    else:
        await set.edit(
            ".gdrivesp <GDrive Qovluq> əmri ilə yeni fayllarınn uploadlanacağı faylı seçə bilərsiz."
        )
...
async def download(gclr):
    ...
    parent_id = GDRIVE_FOLDER_ID
    await gclr.edit("Özəl qovluq ID'si uğurla təmizləndi.")
...
async def show_current_gdrove_folder(event):
    if parent_id:
        ...
        await event.edit(
            f"Codeaz'um faylları [buraya]({folder_link}) uploadlıyır.")
    else:
        await event.edit(
            f"Codeaz'um faylları Google Drive'ın kök qovluğuna uploadlıyır.\
...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(BOTLOG_CHATID) as conv:
        await conv.send_message(
            f"Bu linke get və kodu kopyalayıb cavabla: {authorize_url}"
        )
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Codeaz işlədilərək yükləndi.",
        "mimeType": mime_type,
    }
    if parent_id:
...
async def create_directory(http, directory_name, parent_id):
    ...
    drive_service.permissions().insert(fileId=file_id,
                                       body=permissions).execute()
    LOGS.info("Yaradıldı Gdrive Qovluq:\nAd: {}\nID: {} ".format(
...
CmdHelp('gdrive').add_command(
    ...
    'gsetclear', None, 'Hal-hazırda işlədilən upload qovluğunu göstərər.'
).add_command(
