...
async def dil(event):
    ...
    if search(r"y[uü]kle|install", komut):
        ...
        if event.is_reply:
            ...
            if ((len(reply.file.name.split(".")) >= 2) and (not reply.file.name.split(".")[1] == "codjson")):
                return await event.edit("`Xaiş keçərli bir` **CODJSON** `faylı verin!`")
        ...
        if event.is_reply:
            reply = await event.get_reply_message()
            if ((len(reply.file.name.split(".")) >= 1) and (not reply.file.name.split(".")[1] == "codjson")):
                return await event.edit("`Xaiş keçərli bir` **CODJSON** `faylı verin!`")
        ...
        await event.edit(
            ...
            f"\n\nDigər dillər üçün @CODDil kanalına baxa bilərsiz."
        )
