...
async def upstream(ups):
    "For .yoxlama command, check if the bot is up to date, update if specified"
    ...
    if not await is_off_br(ac_br):
        await ups.edit(
            f'**[Yoxlama]:**` Looks like you are using your own custom branch ({ac_br}). '
    ...
    if conf != "now":
        ...
        await ups.respond('`Use the \".yoxlama now\" command to yoxlama`')
        return
