...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @Codeaz - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @Codeaz - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/Texnocom/Codeaz.git")

# Konsol
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///cod.db")
...
# Last.fm
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@Codeaz | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@Codeaz Paketi")
...
if os.path.exists("codbrain.check"):
    os.remove("codbrain.check")
else:
    LOGS.info("Braincheck faylı yoxdur, getirilir...")

URL = 'https://raw.githubusercontent.com/Texnocom/Codeaz/master/codbrain.check'
with open('codbrain.check', 'wb') as load:
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən ` @Codeaz`! Mən sahibimə (`@{me.username}`) kömək olmaq üçün varam, yəni sənə köməkçi ola bilmərəm :/ Ama sən da bir Codeaz quraşdıra bilərsən; Kanala bax` @DTOUserBot')
            else:
                await event.reply(f'`C O D E A Z`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@Codeaz":
                rev_text = query[::-1]
                ...
                result = builder.article(
                    "@Codeaz",
                    text="""@Codeaz'u işlətməyi yoxlayın!
Hesabınızı bot'a çevirə bilərsiz və bunları işlədə bilərsiz. Unutmayın, siz başqasının botunu idarə edə bilmərsiz! Altdakı GitHub adresindən bütün qurulum haqda məlumat var.""",
                    buttons=[
                        [custom.Button.url("Kanala Qatıl", "https://t.me/Codeaz"), custom.Button.url(
                            "Qrupa Qatıl", "https://t.me/info_az")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Texnocom/Codeaz")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Mənim mesajlarımı düzəltməyə çalışma! Özünə bir @Codeaz qur.", cache_time=0, alert=True)
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@Codeaz":
                ...
                result = await builder.article(
                    f"Xaiş sadəcə .kömek əmri ilə işladin",
                    text=f"**Allah Azərbaycanları qorusun** [Codeaz](https://t.me/Codeaz) __əla işləyir ⚡__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELPTR)}`\n**Səhifə:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
            elif query.startswith("http"):
...
Hesabınızı bot'a çevirə bilərsiz və bunları işlədə bilərsiz. Unutmayın, siz başqasının botunu idarə edə bilmərsiz! Altdakı GitHub adresindən bütün qurulum haqda məlumat var.""",
        ...
        async def sayfa(event):
            ...
            await event.edit(
                f"**Allah Azərbaycanları qorusun** [Codeaz](https://t.me/Codeaz) __əla işləyir ⚡__\n\n**Yüklənən Modil Sayı:** `{len(CMD_HELPTR)}`\n**Səhifə:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
